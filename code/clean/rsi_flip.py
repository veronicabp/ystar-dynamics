...
def restrict_data(row, control_dict, text="", restrict_price=False, remove_previous=False, connect_all=False, radii=[0.1,0.5] + [i for i in range(1,21)]):
	# start = time.time()
	...
	# Restrict price 
	if restrict_price:
		controls = controls.loc[abs(controls['L_log_price_tres'] - row.L_log_price_tres)<=0.1]
	...
	for radius in radii:
		...
		if connected or (radius==radii[-1] and uf.are_connected(row.date, row.L_date)): # If we're at the max radius and the extension window dates are connected, build RSI for those dates
...
def rsi(df, price_var='d_log_price', dummy_vars=[], weight_col=None, get_se=False, get_resid=False, add_constant=True):
...
def rsi_wrapper(row, control_dict, case_shiller=True, price_var='d_log_price', restrict_price=False, add_constant=True, radii=[0.1,0.5] + [i for i in range(1,21)]):

	text = ""
	controls, dates, _, radius = restrict_data(row, control_dict, restrict_price=restrict_price, radii=radii, text=text)
	...
	if case_shiller:
		controls['weight'] = 1/(controls['b_cons'] + controls['b_years_held']*controls['years_held'] + controls['b_distance']*controls['distance'])
		if len(controls[controls['weight']<=0]) > 0:
			print(f"ERROR: Negative weights")
			controls = controls[controls['weight']>0]
		params, constant, summary = rsi(controls, dummy_vars=dummy_vars, price_var=price_var, weight_col='weight', add_constant=add_constant)
	else:
		params, constant, summary = rsi(controls, dummy_vars=dummy_vars, price_var=price_var, add_constant=add_constant)

	d_rsi = params[dates.index(row['date'])] -  params[dates.index(row['L_date'])] + constant
...
def apply_rsi(extensions, control_dict, price_var='d_log_price', case_shiller=True, connect_all=False, add_constant=True):
	extensions[['d_rsi','num_controls','radius']] = extensions.apply(lambda row: rsi_wrapper(row, control_dict, price_var=price_var, case_shiller=case_shiller, add_constant=add_constant), axis=1, result_type="expand")
	return extensions

def apply_rsi_residuals(extensions, control_dict, price_var='d_log_price', case_shiller=False, connect_all=False, add_constant=True, radii=[0.1,0.5] + [i for i in range(1,21)]):
	# start = time.time()
	# text = ""
	dfs = [pd.DataFrame()]
...
def apply_rsi_full(extensions, control_dict, price_var='d_log_price', case_shiller=False, connect_all=False, add_constant=True, radii=[0.1,0.5] + [i for i in range(1,21)]):
	...
	for row in extensions.itertuples():
		controls, dates, text, radius = restrict_data(row, control_dict, radii=radii, connect_all=connect_all)
		if not dates:
			continue

		dummy_vars = [f'd_{date}' for i, date in enumerate(dates) if i!=0]
		params, se, constant, summary = rsi(controls, dummy_vars=dummy_vars, get_se=True)
		N = len(controls)
...
def process_chunk(chunk):
	extensions, control_dict, case_shiller, connect_all, add_constant, price_var, func = chunk 
	output = func(extensions, control_dict, case_shiller=case_shiller, connect_all=connect_all, price_var=price_var, add_constant=add_constant) 
	return output
...
def get_rsi(extensions, controls, price_var='d_log_price', case_shiller=True, connect_all=False, add_constant=True, func=apply_rsi, start_date=1995, end_date=2023, duration_margin=5, parallelize=True, groupby='area'):
	n_jobs = 1
...
def load_data(folder):
	...
	len_df = len(df)
	df.drop(df[df.number_years<50].index, inplace=True) # Short lease - likely to be a commercial lease

	max_dur = df[df.extension].duration2023.max() + 5
	df.drop(df[(~df.extension)&(df.duration>max_dur)].index, inplace=True) # Duration is too long to be a control

	df.drop(df[df.not_valid_ext].index, inplace=True) # Weird extension-like case that we do not know how to classify

	df.drop(df[df.L_date_trans.isna()].index, inplace=True) # First transaction of property -- for RSI we need two
	print(f'Dropped {len_df - len(df)} impossible controls.')

	# Keep only relevant columns
	# df = df[['property_id', 'date_trans', 'L_date_trans', 'years_held', 'year', 'L_year', 'quarter', 'L_quarter', 'month', 'L_month', 'log_price', 'L_log_price', 'd_log_price', 'area', 'outcode', 'postcode', 'extension', 'extension_amount', 'latitude', 'longitude', 'number_years', 'date_from', 'date_registered', 'num_trans', 'lpa_code'] + [col for col in df.columns if 'duration' in col]]
	df = df[['property_id', 'date_trans', 'L_date_trans', 'year', 'L_year', 'quarter', 'L_quarter', 'years_held', 'd_log_price', 'area', 'extension', 'latitude', 'longitude', 'duration2023']]
...
def construct_rsi(data_folder, start_date=1995*4 + 1, end_date=2024*4 + 1):
...
def update_rsi(data_folder, prev_data_folder, original_data_folder, start_date=1995*4 + 1, end_date=2024*4 + 6):
	df = load_data(data_folder)
	...
	old_df = pd.read_pickle(os.path.join(prev_data_folder,'clean','leasehold_flats.p'))
	extensions = df[df.extension].merge(old_df[['property_id','date_trans']], on=['property_id','date_trans'], how='left', indicator=True)
	extensions = extensions[(extensions._merge=='left_only')|((extensions.date_trans.max()-extensions.date_trans).dt.days<365)].copy()
	controls = df.drop(df[(df.extension)].index)

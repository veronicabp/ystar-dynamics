...
def convert_to_gss(df):
	ashe_codes = pd.read_csv(os.path.join(raw_folder, 'ons', 'ashe','ashe_codes.csv'))
	df = df.merge(ashe_codes, left_on='Code', right_on='ashe_code', how='inner')
	df = df.drop(columns=['Code','ashe_code'])
	return df

def convert_to_2021_codes(df):
	gss_map = pd.read_stata(os.path.join(working_folder, 'map_gss_over_time.dta'))
	for year in range(2008,2021):
		...
		df = df.groupby(['gss_code',f'gss_code{year}']).agg({f'earn':'mean', 'num_jobs':'mean', 'Description':'first'}).reset_index()

		# Get weighted mean
		df.loc[df.num_jobs==0, 'num_jobs'] = 0.0001
		df['earn'] = df['earn'] * df['num_jobs']
		df = df.groupby('gss_code').agg({f'earn':sum_with_nan, 'num_jobs':sum_with_nan, 'Description':'first'}).reset_index()
		df['earn'] = df['earn']/df['num_jobs']
	return df
...
if __name__=="__main__":
	...
	dirs = [os.path.join(ashe_folder, d) for d in os.listdir(ashe_folder) if os.path.isdir(os.path.join(ashe_folder, d))]
	dfs = []
	...
	for d in tqdm(dirs):
		...
		earnings = pd.read_excel(earnings_file_path, sheet_name='Male Full-Time', header=4)
		earnings = earnings[~earnings['Code'].isna()]
		earnings = earnings[~earnings['Description'].isin(['Inner London', 'Outer London'])]
		...
		if year<=2011:
			earnings = convert_to_gss(earnings)
		else:
			earnings = earnings.rename(columns={'Code':'gss_code'})

		earnings = convert_to_2021_codes(earnings)
		earnings['year'] = year
	...
	df = pd.concat(dfs)
	df.to_stata(os.path.join(working_folder, 'ashe_earnings.dta'), write_index=False)

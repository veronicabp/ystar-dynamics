...
def drop_duplicates(df):
	...
	df['duration2023'] = df['number_years'] - years_between_dates(pd.Period('2023-01-01', freq='D') - df['date_from'])
	df['min_duration2023'] = df.groupby('merge_key')['duration2023'].transform('min')
	df = df[~((df['min_duration2023'] > 300) & (df['duration2023'] != df['min_duration2023']))].copy()
...
def convert_dates(df, format='DD-MM-YYYY', tags=['from', 'to', 'registered']):
	for tag in tags:    
		...
		if format=='DD-MM-YYYY':
			df[f'year_{tag}'] = pd.to_numeric(df[f'date_{tag}'].str.slice(6,10), errors='coerce')
			df[f'month_{tag}'] = pd.to_numeric(df[f'date_{tag}'].str.slice(3,5), errors='coerce')
			df[f'day_{tag}'] = pd.to_numeric(df[f'date_{tag}'].str.slice(0,2), errors='coerce')
		elif format=='MM-DD-YYYY':
			df[f'year_{tag}'] = pd.to_numeric(df[f'date_{tag}'].str.slice(6,10), errors='coerce')
			df[f'month_{tag}'] = pd.to_numeric(df[f'date_{tag}'].str.slice(0,2), errors='coerce')
			df[f'day_{tag}'] = pd.to_numeric(df[f'date_{tag}'].str.slice(3,5), errors='coerce')
		else:
		...
		df['period'] = df[f'year_{tag}'].fillna(0).astype(int).astype(str) + '-' + df[f'month_{tag}'].fillna(0).astype(int).astype(str).str.zfill(2) + '-' + df[f'day_{tag}'].fillna(0).astype(int).astype(str).str.zfill(2)
		df.loc[(df[f'year_{tag}'].isna())|(df[f'month_{tag}'].isna())|(df[f'day_{tag}'].isna()), 'period'] = np.nan
...
def extract_terms(df, n_jobs=int(os.cpu_count()) - 2):
	...
	df.loc[(df.date_from.isna())&(~df.date_to.isna())&(~df.number_years.isna()), 'date_from'] = df['date_to'] - df['number_years'].fillna(0).astype(int)*365

	df.loc[(df.date_from.isna())&(df.number_years.isna())&(~df.date_to.isna()), 'number_years'] =  years_between_dates(df['date_to'] - df['date_registered'])
	df.loc[(df.date_from.isna())&(~df.date_to.isna()), 'date_from'] = df.date_registered

	df = df[(~df.date_from.isna())&(~df.number_years.isna())&(df.number_years>=0)]

	df = drop_duplicates(df)
	df.drop(columns=['mean_duration2023', 'min_duration2023', 'sd_duration2023'], inplace=True)
...
def clean_purchased_leases(data_folder):
	...
	closed_leases = closed_leases.rename(columns={'TITLE_CLOS_DATE' : 'date_registered', 'TERM':'term', 'CLOS_YEAR':'year_registered', 'TRANSACTION_ID':'unique_id', 'DEED_DATE':'date_deed'})
	...
	file = os.path.join(data_folder, 'raw', 'hmlr', 'purchased_titles', 'Princeton Lease Information for Issue.csv')
	other_leases = pd.read_csv(file, encoding='latin1')
	other_leases = other_leases.rename(columns={'Date Registered' : 'date_registered', 'Registered Lease Details':'term', 'Client Reference':'unique_id'})
	other_leases.columns = other_leases.columns.str.lower()

	other_leases.drop_duplicates(subset='unique_id', inplace=True)

	other_leases['date_registered'] = pd.to_datetime(other_leases['date_registered'], format='mixed')
	other_leases['date_registered'] = other_leases['date_registered'].dt.strftime('%d/%m/%Y')
	...
	purchased_leases = convert_dates(purchased_leases, tags=['registered', 'deed', 'from', 'to'])

	# If lease has an expiration date but not an origination date, use the deed date as the origination date 
	purchased_leases['flag'] = (purchased_leases.date_from.isna())&(~purchased_leases.date_to.isna())&(~purchased_leases.date_deed.isna())
	purchased_leases.loc[(purchased_leases.flag)&(purchased_leases.number_years.isna()), 'number_years'] =  years_between_dates(purchased_leases['date_to'] - purchased_leases['date_deed'])
	purchased_leases.loc[(purchased_leases.flag)|((purchased_leases.date_from.isna())&(~purchased_leases.number_years.isna())), 'date_from'] = purchased_leases.date_deed

	purchased_leases = purchased_leases[~((purchased_leases.number_years.isna())|(purchased_leases.number_years<0)|(purchased_leases.date_from.isna()))].copy()
...
def identify_extensions(base_df, new_df):
	base_df.rename(columns={x: f'{x}_prev' for x in base_df.columns if x!='id'}, inplace=True)
	df = new_df.merge(base_df, on='id', indicator=True, how='outer')
	
	df['extension'] = (df._merge=='both')&(df.duration2023 - df.duration2023_prev>30)
...
def rename_columns(df):
	...
	df.rename(columns={'os uprn':'uprn', 'associated property description id':'id', 'unique identifier':'unique_id'}, inplace=True)
	return df

def keep_columns(df, additional_cols=[]):
	cols_to_keep = ['merge_key', 'term', 'id', 'uprn', 'unique_id', 'postcode', 'extension', 'extension_amount', 'duration2023'] + [col for col in df.columns if 'date' in col or 'number_years' in col] + additional_cols
	cols_to_keep = list(set(cols_to_keep) & set(df.columns))
	return df[cols_to_keep]

def get_new_leases(df_old, df_new):
	new_leases = df_new.merge(df_old[['id','term']].drop_duplicates(), on=['id','term'], how='left', indicator=True)
...
def clean_leases(data_folder):
	...
	purchased_leases.to_pickle(os.path.join(data_folder, 'working', 'purchased_lease_data.p'))
	...
	leases_may2023 = rename_columns(pd.read_csv(os.path.join(data_folder, 'raw', 'hmlr', 'LEASES_FULL_2023_05.csv')))
	new_leases = rename_columns(pd.read_csv(os.path.join(data_folder, 'raw', 'hmlr', 'LEASES_FULL_2024_02.csv')))
...
def update_leases(data_folder, prev_data_folder):
	...
	new_leases_file = [file for file in sorted(os.listdir(data_folder_path)) if 'LEASES_FULL_' in file][-1]
	old_leases_file = [file for file in sorted(os.listdir(prev_data_folder_path)) if 'LEASES_FULL_' in file][-1]

	new_leases = rename_columns(pd.read_csv(os.path.join(data_folder_path, new_leases_file)))
	old_leases = rename_columns(pd.read_csv(os.path.join(prev_data_folder_path, old_leases_file)))
	...
	old_leases = pd.read_pickle(os.path.join(prev_data_folder, 'working', 'lease_data.p'))
